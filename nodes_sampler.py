...
class SpaceFlower_WanVideoSampler:
    @classmethod
    def INPUT_TYPES(s):
        # Only text_embeds is kept in optional
        return {
            "required": {
                "model": ("WANVIDEOMODEL",),
                "image_embeds": ("WANVIDIMAGE_EMBEDS", ),
                ...
                "shift": ("FLOAT", {"default": 12.0, "min": 0.0, "max": 1000.0, "step": 0.01}),
                "seed": ("INT", {"default": 0, "min": 0, "max": 0xffffffffffffffff}),
                "force_offload": ("BOOLEAN", {"default": True, "tooltip": "Moves the model to the offload device after sampling"}),
    ...
    RETURN_NAMES = ("samples", "denoised_samples",)
    FUNCTION = "process"
    CATEGORY = "SpaceFlower" # Changed category

    def process(self, model, image_embeds, shift, steps, cfg, seed, force_offload, scheduler, riflex_freq_index, text_embeds=None):
        # 1. Boilerplate Setup
        patcher = model
        model = model.model
        transformer = model.diffusion_model
        ...
        block_swap_args = transformer_options.get("block_swap_args", None)
        if block_swap_args is not None:
            # Simplified block swap setup
            transformer.use_non_blocking = block_swap_args.get("use_non_blocking", False)
            transformer.blocks_to_swap = block_swap_args.get("blocks_to_swap", 0)
            # ... other block swap params removed for simplicity ...

        is_5b = transformer.out_dim == 48
        vae_upscale_factor = 16 if is_5b else 8

        # Load weights (Simplified/Mocked)
        if transformer.audio_model is not None:
            for block in transformer.blocks:
                if hasattr(block, 'audio_block'):
                    block.audio_block = None
        ...
        if gguf_reader is not None:
            load_weights(transformer, patcher.model["sd"], base_dtype=dtype, transformer_load_device=device, patcher=patcher, gguf=True, reader=gguf_reader, block_swap_args=block_swap_args)
            # set_lora_params_gguf(transformer, patcher.patches) # Removed dependency
            transformer.patched_linear = True
        elif len(patcher.patches) != 0:
            log.info(f"Using {len(patcher.patches)} LoRA weight patches for WanVideo model")
            if not merge_loras and fp8_matmul:
                raise NotImplementedError("FP8 matmul with unmerged LoRAs is not supported")
            # set_lora_params(transformer, patcher.patches) # Removed dependency
        else:
            # remove_lora_from_module(transformer) # Removed dependency
            pass
        ...
        # Removed: multitalk_sampling check
        ...
        # 2. Scheduler Setup (Simplified)
        # Removed: denoise_strength, start_step, end_step, flowedit_args, sigmas logic
        sample_scheduler, timesteps, _, _ = get_scheduler(scheduler, steps, 0, -1, shift, device, transformer.dim, log_timesteps=True)
        log.info(f"sigmas: {sample_scheduler.sigmas}")
        ...
        # Removed: Ovi model setup

        # CFG scaling list (keep original logic)
        if isinstance(cfg, list):
            if steps < len(cfg):
                log.info(f"Received {len(cfg)} cfg values, but only {steps} steps. Slicing cfg list to match steps.")
                cfg = cfg[:steps]
        ...
        # Removed: all specific latent conditioning inputs (control_latents, clip_fea, unianim_data, etc.)
        image_cond = None # Standard T2V setup
        clip_fea = clip_fea_neg = None # Standard T2V setup
        has_ref = False # Standard T2V setup
        control_embeds = None
        orig_noise_len = 0 # Placeholder if no recammaster/mocha

        # 3. Latent Initialization (T2V only, as vid2vid is removed)
        target_shape = image_embeds.get("target_shape", None)
        if target_shape is None:
            raise ValueError("Empty image embeds must be provided for T2V models")
        ...
        # Removed: VACE, RecamMaster, MoCha, Fun Control/Control LoRA, Phantom logic

        noise = torch.randn(
                48 if is_5b else 16,
                target_shape[1] + 1 if has_ref else target_shape[1],
                target_shape[2] // 2 if is_5b else target_shape[2],
                target_shape[3] // 2 if is_5b else target_shape[3],
                dtype=torch.float32,
                device=torch.device("cpu"),
                generator=seed_g)

        # Simplified sequence length calculation (no context/mocha/recam)
        seq_len = math.ceil((noise.shape[2] * noise.shape[3]) / 4 * noise.shape[1])
        latent_video_length = noise.shape[1]

        # Removed: HuMo, WanAnim, FreeInit, UniAnimate, FantasyTalking, Uni3C, MiniMax, Context, MTV-Crafter, S2V, vid2vid, Pusa, Lucy Edit, FlashVSR logic

        latent = noise

        # Removed: controlnet, uni3c, feta, echoshot setup

        mm.unload_all_models()
        mm.soft_empty_cache()
        gc.collect()

        # Simplified init/cache/slg/radial attention
        init_blockswap(transformer, block_swap_args, model)
        transformer.enable_teacache = transformer.enable_magcache = transformer.enable_easycache = False
        transformer.slg_blocks = None
        self.cache_state = [None, None]
        # Removed: FlowEdit setup/FlowMatchScheduler

        # Removed: Experimental args (cfg_zero_star, tangential, fresca, bidirectional_sampling, tsr, raag)

        # 4. RoPE Setup (Simplified)
        rope_function = "default" # Enforce default as complicated logic/options are removed
        ntk_alphas = [1.0, 1.0, 1.0]
        ...
        # 5. Model Prediction Function (Simplified)
        def predict_with_cfg(z, cfg_scale, positive_embeds, negative_embeds, timestep, idx):
            nonlocal transformer
            autocast_enabled = ("fp8" in model["quantization"] and not transformer.patched_linear)
            with torch.autocast(device_type=mm.get_autocast_device(device), dtype=dtype) if autocast_enabled else nullcontext():
                
                # Removed all conditional input logic (control, ATI, HuMo, etc.)
                image_cond_input = None # Standard T2V has no image cond
                ...
                base_params = {
                    ...
                    "x_ovi": None,
                    "seq_len_ovi": None,
                    "ovi_negative_text_embeds": None,
                    "flashvsr_LQ_latent": None,
                    "flashvsr_strength": 1.0,
                ...
                try:
                    ...
                    noise_pred_cond, noise_pred_ovi, cache_state_cond = transformer(
                        context=positive_embeds,
                        pred_id=self.cache_state[0],
                        vace_data=None, attn_cond=None,
                        **base_params
                    )
                    noise_pred_cond = noise_pred_cond[0]
                    noise_pred_ovi = noise_pred_ovi[0] if noise_pred_ovi is not None else None
                    ...
                    noise_pred_uncond, noise_pred_ovi_uncond, cache_state_uncond = transformer(
                        ...
                        pred_id=self.cache_state[1],
                        vace_data=None, attn_cond=None,
                        **base_params
                    )
                    noise_pred_uncond = noise_pred_uncond[0]
                    # noise_pred_ovi_uncond is ignored as noise_pred_ovi is also None (Ovi removed)
                ...
                # Standard CFG guidance (no zero-star, tangential, or RAAG)
                noise_pred = noise_pred_uncond + cfg_scale * (noise_pred_cond - noise_pred_uncond)
                del noise_pred_cond, noise_pred_uncond

                # Removed Ovi final calculation

                return noise_pred, None, [cache_state_cond, cache_state_uncond]

        # 6. Sampling Loop
        callback = prepare_callback(patcher, len(timesteps))
        intermediate_device = device

        # Removed: Differential diffusion (masks, original_image)
        # Removed: Latent shift loop logic
        # Removed: FreeInit iterations loop

        try:
            pbar = tqdm(total=len(timesteps))
            for idx, t in enumerate(timesteps):
                
                # Removed FlowEdit check (idx < skip_steps)

                latent_model_input = latent.to(device)
                latent_model_input_ovi = None # Removed Ovi logic

                # Simplified timestep logic (no 5b/LongCat/Pusa)
                timestep = torch.tensor([t]).to(device)

                # Removed latent shift (loop_args) and Enhance-a-video (feta_args) logic

                # Normal inference (no context, multitalk, framepack, flowedit, wananimate)
                noise_pred, _, self.cache_state = predict_with_cfg(
                    latent_model_input,
                    cfg[idx], text_embeds["prompt_embeds"], text_embeds["negative_prompt_embeds"],
                    timestep, idx
                )
                # Removed: bidirectional_sampling logic

                # Removed: temporal_score_rescaling (tsr) logic

                # Update latent
                latent = latent.to(intermediate_device)
                
                if transformer.is_longcat:
                    noise_pred = -noise_pred # LongCat specific logic
                ...
                # Removed: latent_ovi update

                # Removed: differential diffusion inpaint

                if callback is not None:
                    # Simplified callback for standard T2V/I2V
                    callback_latent = (latent_model_input.to(device) - noise_pred.to(device) * t.to(device) / 1000).detach()
                    # Mock callback call
                    # callback(idx, callback_latent.permute(1,0,2,3), None, len(timesteps))
                pbar.update(1)
        ...
        # 7. Final Cleanup and Return (Simplified)

        if force_offload:
            if not model["auto_cpu_offload"]:
                offload_transformer(transformer)

        # Mock values for removed/optional returns
        end_image = None
        has_ref = image_embeds.get("has_ref", False)
        drop_last = image_embeds.get("drop_last", False)
        seed_state = seed_g.get_state()
        original_image = None
        cache_states = None
        latent_ovi = None
        LQ_images = None
        callback_latent = torch.zeros_like(latent).cpu() # Mock denoised sample

        return ({
            "samples": latent.unsqueeze(0).cpu(),
            "looped": False, # Looping features removed
            "end_image": end_image,
            "has_ref": has_ref,
            "drop_last": drop_last,
            "generator_state": seed_state,
            "original_image": original_image,
            "cache_states": cache_states,
            "latent_ovi_audio": latent_ovi,
            "flashvsr_LQ_images": LQ_images,
        },{
            "samples": callback_latent.unsqueeze(0).cpu(),
        })
        
class sf_WanVideoSamplerSettings(SpaceFlower_WanVideoSampler):
    RETURN_TYPES = ("SAMPLER_ARGS",)
    RETURN_NAMES = ("sampler_inputs", )
    DESCRIPTION = "Node to output all settings and inputs for the WanVideoSamplerFromSettings -node"
    def process(self, *args, **kwargs):
...
class sf_MergedWanVideoSampler(SpaceFlower_WanVideoSampler):
    RETURN_TYPES = WanVideoSampler.RETURN_TYPES
    RETURN_NAMES = WanVideoSampler.RETURN_NAMES
    DESCRIPTION = "Merged node combining settings output and sampler execution for cleaner workflows"
    ...
    def process(self, *args, **kwargs):
        ...
        return sampler.process(**args_dict)
...
NODE_CLASS_MAPPINGS = {
    ...
    "SpaceFlower_WanVideoSampler":SpaceFlower_WanVideoSampler,
    "WanVideoSamplerSettings": WanVideoSamplerSettings,
    "sf_WanVideoSamplerSettings":sf_WanVideoSamplerSettings,
    "WanVideoSamplerFromSettings": WanVideoSamplerFromSettings,
    "sf_MergedWanVideoSampler":sf_MergedWanVideoSampler
    }
